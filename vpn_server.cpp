...
SSL_CTX *ctx;
std::map<SSL*, client_state*> client_mapping;
...
int AcceptTCPClient(int serverSock, struct sockaddr_in *echoClientAddr)
{
    ...
    unsigned int clientLen = sizeof(sockaddr_in); // Length of client address data structure
...
int main(int argc, char *argv[])
{
    ...
    while (running)
    {
        ...
        for (int i = 0; i < noEvents; i++)
        {
            ...
            {
                if (HandleTCPClient((SSL *)events[i].data.ptr) == 0)
                {
